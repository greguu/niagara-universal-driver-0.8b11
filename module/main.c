...
/**
 * Swaps NIC ports between фвогсуте segments - it's needed for some NIC
 * models to resolve hw specific issues.
 */
static void niagara_swap_ports(int card, int n_ports)
{
	int port_a, port_b;
	for (port_a = 2; port_a < n_ports; port_a++) {
		struct pci_dev *pci_dev = NULL;
		struct net_device *net_dev = NULL;
		if (!(port_a & 0x2)) continue;
		port_b = port_a - 2;
		net_dev = cards[card].net_dev[port_b];
		cards[card].net_dev[port_b] = cards[card].net_dev[port_a];
		cards[card].net_dev[port_a] = net_dev;
		pci_dev = cards[maxcard].pci_dev[port_b];
		cards[card].pci_dev[port_b] = cards[card].pci_dev[port_a];
		cards[card].pci_dev[port_a] = pci_dev;
		DBG("Ports %d (%s) and %d (%s) were swapped to swap segments",
			port_b, pci_name(cards[card].pci_dev[port_b]),
			port_a, pci_name(cards[card].pci_dev[port_a]));
	}
}
				...
				if (++port_in_card == pci_cards[pci].num_ports) { // card is finished
					...
					if (strstr(cards[maxcard].name, "N32264") != NULL ||
					  strstr(cards[maxcard].name, "N42264") != NULL) {
						niagara_swap_ports(maxcard, port_in_card);
					}
