...
static heartbeat_t heartbeat[MAX_CARD][MAX_PORT_PER_CARD/2];
static volatile unsigned long heartbeat_active;
...
typedef void (*timer_proc_t)(unsigned long data);

static inline void niagara_hb_timer_setup(struct timer_list *timer, timer_proc_t handler)
{
	init_timer(timer);
	timer->expires = jiffies + 1;
	timer->function = handler;
	timer->data = (unsigned long) container_of(timer, heartbeat_t, timer);
	add_timer(timer);
}

#else

typedef void (*timer_proc_t)(struct timer_list *t);
...
{
	heartbeat_t *hb_seg = (heartbeat_t *) data;
#else
static void timer_proc(struct timer_list *t)
{
	heartbeat_t *hb_seg = from_timer(hb_seg, t, timer);
#endif
	const int id = ((u8*)hb_seg - (u8*)&heartbeat)/sizeof(heartbeat_t);
	const int card = id / (MAX_PORT_PER_CARD/2);
	const int segment = id % (MAX_PORT_PER_CARD/2);
	if (hb_seg->period == 0)
		hb_seg->period = DEFAULT_HEARTBEAT_PERIOD;
	if (cpld_kick(card, segment))
		mod_timer(&hb_seg->timer, jiffies + 1);
	else
		mod_timer(&hb_seg->timer, jiffies + hb_seg->period * HZ / 10);
}
...
{
	*value = !!test_bit(card*MAX_PORT_PER_CARD/2 + segment, &heartbeat_active);
	return 0;
}
static int set_heartbeat(int card, int segment, unsigned value)
{
	struct timer_list *timer = &heartbeat[card][segment].timer;
	switch (value) {
	case 1:
		if (test_and_set_bit(card*MAX_PORT_PER_CARD/2 + segment, &heartbeat_active)) break;
		niagara_hb_timer_setup(timer, timer_proc);
		break;
	case 0:
		if (!test_and_clear_bit(card*MAX_PORT_PER_CARD/2 + segment, &heartbeat_active)) break;
		del_timer(timer);
		break;
...
{
	*value = heartbeat[card][segment].period;
	return 0;
}
static int set_heartbeat_period(int card, int segment, unsigned value)
{
	heartbeat[card][segment].period = value;
	return 0;
}
//kick
